...
WRITE_CAPABILITY_SCHEMA: dict[str, Any] = {
    ...
    "function": {
        ...
        "description": (
            "Write or update a capability as a Python file in "
            "capabilities/, register it in the DB, and hot-reload it. "
            "Also use this to fix a broken capability — just call it "
            "again with corrected code.\n\n"
            "The code MUST define one async function matching the "
            "capability name with this exact signature:\n\n"
            "```python\n"
            "from typing import Any\n"
            "import asyncpg\n\n"
            "async def my_capability("
            "params: dict[str, Any], pool: asyncpg.Pool) -> str:\n"
            "    import json\n"
            "    name = params.get('name', 'world')\n"
            "    return json.dumps({'greeting': f'hello {name}'})\n"
            "```\n\n"
            "Rules:\n"
            "- Two arguments: params (dict from tool schema) and pool "
            "(DB connection pool). Always include both.\n"
            "- Must return a string. Use json.dumps() for structured "
            "data.\n"
            "- Use httpx for HTTP (not requests). httpx, json, asyncio,"
            " and stdlib are available.\n"
            "- If you need an uninstalled package, tell the user to "
            "run `uv add <package>`.\n"
            "- Use execute_sql to create tables or query data."
        ),
        "parameters": {
            "type": "object",
            "properties": {
